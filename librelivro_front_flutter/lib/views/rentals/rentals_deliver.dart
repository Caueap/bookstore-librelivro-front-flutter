...
class _RentalDeliverState extends State<RentalDeliver> {
  ...
  getRentalByIdInModify() {
    ...
    rentalService.getRentalById(widget.id ?? 0).then((response) {
      setState(() {
        isLoading = false;
      });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(title: Text('Devolver Livro')),
        body: Padding(
                  ...
                  child: Column(
                    children: [
                      TextFormField(
                        ...
                        decoration:
                            InputDecoration(hintText: 'Data de devolução'),
                        onTap: () async {
                        ...
                        child: ElevatedButton(
                          onPressed: () async {
                            if (formKey.currentState!.validate()) {
                              ...
                              final result = await rentalService.updateRental(
                                  widget.id!, rental);
